        ...
        <div className="flex justify-between items-center h-20">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            <div className="relative">
              ...
              {/* Badge statut */}
              {isAuthenticated && (
                <div className={`absolute -top-1 -right-1 w-4 h-4 ${user?.role === 'admin' ? 'bg-purple-500' : 'bg-green-500'} rounded-full border-2 border-slate-900 animate-pulse`}></div>
              )}
            </div>
            ...
            <div className="hidden md:block">
              ...
              <div className="text-xs text-slate-500 -mt-1 font-medium">
                {isAuthenticated ? (
                  user?.role === 'admin' ? '⚡ Admin Panel' : '🤖 IA Assistant'
                ) : (
          ...
          <ul className="hidden lg:flex items-center gap-1 list-none m-0 p-0">
            ...
            {/* Menu principal (toujours affiché) */}
            ...
            {isAuthenticated ? (
              <>
                ...
                <div className="flex items-center gap-3 px-4 py-2 bg-slate-800/50 rounded-xl border border-slate-700">
                  <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${user?.role === 'admin' ? 'from-purple-500 to-pink-500' : 'from-blue-500 to-purple-500'} flex items-center justify-center font-bold text-sm`}>
                  ...
                  <div className="hidden xl:block">
                    ...
                    <div className={`text-sm font-bold text-transparent bg-clip-text bg-gradient-to-r ${user?.role === 'admin' ? 'from-purple-400 to-pink-400' : 'from-blue-400 to-purple-400'}`}>
                      {user?.name}
                    </div>
                  </div>
                </div>
                ...
                {user?.role === 'client' && (
                  <Link 
                    to="/profile"
                    className="p-2.5 bg-slate-800/50 hover:bg-slate-700 rounded-xl border border-slate-700 hover:border-blue-500/50 transition-all duration-300 group"
                    title="Mon profil"
                  >
              ...
              <>
                ...
                <button 
                  ...
                  className="px-5 py-2.5 text-sm font-semibold text-slate-300 hover:text-white hover:bg-slate-800 rounded-xl transition-all duration-300 border border-transparent hover:border-slate-700"
                >
                  Connexion
                </button>
              ...
              {isAuthenticated ? (
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3 px-4 py-3 bg-slate-800/50 rounded-lg">
                    <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${user?.role === 'admin' ? 'from-purple-500 to-pink-500' : 'from-blue-500 to-purple-500'} flex items-center justify-center font-bold`}>
                      {user?.name?.charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <div className="text-xs text-slate-400">
                      ...
                      <div className={`font-bold text-transparent bg-clip-text bg-gradient-to-r ${user?.role === 'admin' ? 'from-purple-400 to-pink-400' : 'from-blue-400 to-purple-400'}`}>
                        {user?.name}
                      </div>
                    </div>
                  </div>
